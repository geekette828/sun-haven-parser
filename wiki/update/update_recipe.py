...
from wiki.shared import recipe_core
from wiki.compare.recipe_fields import RECIPE_FIELD_MAP
    ...
    for title in batch:
        ...
        if not templates:
            for tpl in parsed.filter_templates():
                if tpl.name.strip().lower() == "recipe/none":
                    ...
                    mapped["id"] = matched_json.get("recipe_id", "").strip()
        ...
        for template in templates:
            ...
            for field, expected, actual in diffs:
                ...
                if field == "id" and not matched_json.get("recipe_id"):
                    continue
