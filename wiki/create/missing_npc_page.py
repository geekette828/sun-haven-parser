...
- Builds NPC page text by calling into:
  formatters.pages.npc_page (build_page_wikitext)
...
from formatters.pages import npc_page as create_npc_page

# ---------------------------------------------------------------------------
# NPC file path helpers
# (file I/O lives here, not in the pure formatter)
# ---------------------------------------------------------------------------

_INVALID_FILENAME_CHARS = r'<>:"/\|?*'
_NPC_DIALOGUE_DIR = os.path.join(constants.OUTPUT_DIRECTORY, "Wiki Formatted", "NPC Dialogue")
_NPC_SCHEDULE_DIR = os.path.join(constants.OUTPUT_DIRECTORY, "Wiki Formatted", "NPC Schedules")
...
def get_one_liner_file_path(npc_name: str) -> str:
    return os.path.join(_NPC_DIALOGUE_DIR, f"{_safe_filename(npc_name)} one liners.txt")


def get_cycles_file_path(npc_name: str) -> str:
    return os.path.join(_NPC_DIALOGUE_DIR, f"{npc_name} cycles.txt")
...
def load_schedule_text(npc_name: str) -> str:
    return read_text_file(os.path.join(_NPC_SCHEDULE_DIR, f"{npc_name}_schedule.txt"))
...
def build_npc_page_text(npc_name: str) -> str:
    one_liner_text = read_text_file(get_one_liner_file_path(npc_name))
    cycles_text = read_text_file(get_cycles_file_path(npc_name))
