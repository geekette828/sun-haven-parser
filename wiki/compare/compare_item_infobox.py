...
from wiki.shared import item_infobox_core
from utils import file_utils, text_utils, wiki_utils
...
json_file_path = os.path.join(
    constants.OUTPUT_DIRECTORY, "JSON Data", "items_data.json"
)
output_file = os.path.join(
    constants.OUTPUT_DIRECTORY, "Pywikibot", "item_infobox_compare.txt"
)
debug_log_path = os.path.join(
    constants.DEBUG_DIRECTORY, "pywikibot", "item_infobox_compare_debug.txt"
)
...
# Core decides which keys actually matter based on the infobox template.
# This is only a required fallback argument.
KEYS_TO_CHECK = item_infobox_core.COMMON_KEYS
...
for i in range(0, total, BATCH_SIZE):
    ...
    # Use shared wiki util (PreloadingGenerator + site config)
    ...
    for title in batch:
        ...
        if normalized_title in data:
            ...
            if diffs:
                ...
                for field, exp, act in diffs:
                    debug_lines.append(
                        f"[MISMATCH] {title}    {field}: {exp}/{act}"
                    )
            else:
        ...
        # Variant match
            ...
            for variant in variant_map[normalized_title]:
                if (
                    subtype not in ["Mount", "Pet"]
                    and not re.search(r" \([^)]+\)$", variant)
                ):
                    continue
                ...
                diffs = [
                    ...
                    for d in (diffs or [])
                    if d[0] != "name"
                    and not (d[0] == "statInc" and "»999" in str(d[1]))
                ]
            ...
            if differing:
                ...
                for variant, field, exp, act in differing:
                    ...
                    debug_lines.append(
                        f"     - ({color}) {field}: expected '{exp}' but found '{act}'"
                    )
                    mismatch_entry.append(
                        (f"({color}) {field}", exp, act)
                    )

                if matching:
                    matched_colors = [
                        f"({v.replace(normalized_title, '').strip()})"
                        for v in matching
                    ]
                    mismatch_entry.append(
                        ("MATCH EXACTLY", ", ".join(matched_colors), "")
                    )
            ...
            if matching and not differing:
                ...
                debug_lines.append(
                    f"[VARIANT - MATCH] {title} - {variants}"
                )
    ...
    for title, diffs in mismatches:
        ...
        for field, exp, act in diffs:
            if field == "MATCH EXACTLY":
                ...
                out.write(
                    f"    - {field}: expected '{exp}' but found '{act}'\n"
                )
        out.write("\n")
