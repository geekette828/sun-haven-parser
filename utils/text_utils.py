...
def clean_game_dialogue(text: str) -> str:
    """
    Cleaning pipeline for in-game dialogue extracted from localization files.

    - Removes Unity color / sprite tags
    - Replaces XX with {{PLAYER}}
    - Converts [] and newlines to <br>
    - Strips HTML tags like <i>
    - Preserves wiki markup (''' [[ ]])
    """
    if not text:
        return ""
