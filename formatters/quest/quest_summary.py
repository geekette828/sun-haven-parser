"""
Quest summary formatter — Layer 2 of the pipeline.

Pure formatting function for the quest page summary paragraph.
...
def create_quest_summary(quest: dict, npc: str = "", bulletin: str = "", end_text: str = "") -> str:
    quest_type = resolve_quest_type(quest).lower()
    region = quest.get("region", "").strip()
    name = quest.get("quest_name", "Unnamed Quest")
    objective = text_utils.clean_game_dialogue(quest.get("quest_description", ""))
    description = text_utils.clean_game_dialogue(quest.get("bulletin_board_description", ""))
