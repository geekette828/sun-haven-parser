...
SUMMARIES: dict[tuple[str, str, str], str] = {
    ...
    ("Furniture", "Flooring", ""): "'''ITEM NAME''' is a cosmetic furniture item used to customize the interior [[flooring|floors]] of player homes. When applied, it changes the appearance of the floor in the selected room, allowing the player to decorate their home with different styles, colors, and themes. Flooring is reusable and can be replaced at any time with another flooring item. Unlike outdoor tiles, paths, or patios placed on the farm, flooring is designed specifically for indoor house decoration.\n\nTo use flooring, place the item on the hotbar, enter the room you want to customize, then select and use the item while standing inside. The flooring will immediately be applied to the current room. If the house has multiple rooms, each room can be customized separately by using flooring while inside that room.",
